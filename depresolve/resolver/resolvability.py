...
def is_dep_satisfied(edep, candidates, disregard_setuptools=False,
  ...
  # Else, dep might be satisfied - we have to make sure the version is OK.
    ...
    if len(same_package_dist) != 1:
      raise ValueError('Programming error. Multiple dists of the same package '
          'provided in a candidate list. N=' + str(len(same_package_dist)))
...
def are_fully_satisfied(candidates, edeps, versions_by_package,
    disregard_setuptools=False, report_issue=False):
  """
  ...
  Arguments:
    ...
    4. disregard_setuptools: optional. I dislike this hack. Because for the
       rbtcollins resolver, I'm testing solutions generated by pip installs and
       harvested by pip freeze, I'm not going to get setuptools listed in the
       solution set (pip freeze doesn't list it), so ... for that, I pass in
  ...
  for distkey in candidates:
    ...
    for edep in edeps[distkey]:
      this_dep_satisfied = is_dep_satisfied(edep, candidates,
          disregard_setuptools=disregard_setuptools) # do not use report_issue
